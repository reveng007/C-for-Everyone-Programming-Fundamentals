...
int post_incre(int a[], int *p)
{
    printf("%d\n", *(p++)); 
    // p++ => s1: p = 1000 ==> *p = *(1000) = 5  | s2: p = p + (1 x 4) [as in this case it is array] = 1000 + 4 = 1004 ==> *p = *(1004) = 16
    // so, *(p++) = 5
    printf("%d\n", *p); // In next step, *p = 16
}


int pre_incre(int a[], int *p)
{
    printf("%d\n", *(++p)); 
...
int main()
{
    ...
    int *p = &a[0]; // Suppose base address of the array is 1000
